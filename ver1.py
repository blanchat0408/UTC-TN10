...
def devices(consommation: list, production: list, starttime: int, leavetime: int):
    class Battery:
        SoCmin = 0.2
        SoCmax = 0.8
        Pmax = 5
    ...
    for t in range(23):
        ...
        if diff[t] <= 0:
            ...
            if bat.SoC[t] <= bat.SoCmin:
                ...
                bat.Pdis[t] = round(min(bat.Pmax, abs(diff[t]), (bat.SoC[t] - bat.SoCmin) * bat.cap), 2)
        else:
            # charge battery
            if bat.SoC[t] >= bat.SoCmax:
                bat.Pchar[t] = 0
            else:
                bat.Pchar[t] = round(min(bat.Pmax, diff[t], (bat.SoCmax - bat.SoC[t]) * bat.cap), 2)
...
class Maison(Agent):
    ...
    class SaveData(OneShotBehaviour):
        ...
        async def run(self) -> None:
            import pandas as pd
            import os.path
            data_profil = pd.DataFrame([self.agent.consommation, self.agent.production,
