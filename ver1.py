...
def devices(consommation: list, production: list, starttime: int):
    class Battery:
        SoCmin = 0.2
        SoCmax = 0.8
        Pmax = 5
    ...
    for t in range(23):
            ...
            if ev.Pev_a_char_t > 0:
                ...
                if diff >= 0:
                    bat.Pdis[t] += min(ev.Pev_a_char[t] - min(ev.Pev_a_char[t], diff), bat.Pmax,
                                       (bat.SoC[t]-bat.SoCmin) * bat.cap)
                    diff -= min(diff, ev.Pev_a_char[t])

                else:
                    bat.Pdis[t] += min(ev.Pev_a_char[t], bat.Pmax, (bat.SoC[t]-bat.SoCmin) * bat.cap, abs(diff))
                    diff += bat.Pdis[t]
        ...
        if diff <= 0:
            if bat.SoC[t] <= bat.SoCmin:
                ...
                bat.Pdis[t] += min(bat.Pmax, abs(diff), (bat.SoC[t] - bat.SoCmin) * bat.cap)
        else:
            if bat.SoC[t] >= bat.SoCmax:
                bat.Pchar[t] = 0
            else:
                bat.Pchar[t] = min(bat.Pmax, diff, (bat.SoCmax - bat.SoC[t]) * bat.cap)
    ...
    class Initialization(OneShotBehaviour):
        async def run(self) -> None:
            ...
            self.agent.battery, self.agent.ev, self.agent.consommation = devices(self.agent.consommation,
                                                                                 self.agent.production, starttime)
